<template>
	<view class="yingbing-read-page" ref="yingbingReadPage" @touchstart="touchstart" @touchmove="touchmove" @touchend="touchend">
		...
		<view class="yingbing-read-page-flip" :style="{
			'visibility': options.pageType != 'scroll' ? 'visible' : 'hidden'
		}">
			<!-- #ifndef APP-NVUE -->
			<view
			class="yingbing-flip"
			...
			<!-- #endif -->
				<template v-if="isShow">
					<template v-for="(item, index) in dataReverse" :key="item.dataId + '_flip'">
						<view 
						...
						:style="{
							...
							:style="{
								...
								<template v-else-if="item.type == 'custom'">
									...
									<!-- #endif -->
									<!-- #ifdef APP-NVUE -->
									<view class="flip-custom flip-item-content">
										<rich-text :nodes="parseHtml(item.text)"></rich-text>
									</view>
									<!-- #endif -->
								</template>
		...
		<!-- 滚动模式 -->
		<view class="yingbing-scroll" :style="{
			'background': options.bgColor,
			...
			@scrolltoupper="scrolltoupper">
				...
				:style="{
					...
					<template v-else-if="item.type == 'custom'">
						<!-- #ifndef APP-NVUE -->
						<view class="scroll-custom scroll-item-content" v-html="item.text">
						</view>
						<!-- #endif -->
						<!-- #ifdef APP-NVUE -->
						<view class="scroll-custom scroll-item-content">
							<rich-text :nodes="parseHtml(item.text)"></rich-text>
						</view>
						<!-- #endif -->
					</template>
		...
		<view class="yingbing-loading" v-if="initLoading" :style="{background: options.bgColor}" @tap="reload">
			<yb-loading :size="40" :visible="initLoading" :color="options.color" :title="loadingText"></yb-loading>
		</view>
	</view>
</template>
